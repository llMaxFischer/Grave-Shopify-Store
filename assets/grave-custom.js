...
(function() {
  ...
  /* Clear on page load (after theme JS runs) */
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      setTimeout(clearSizeSelection, 100);
    });
  } else {
    setTimeout(clearSizeSelection, 100);
  }

  /* Also clear after any AJAX/fancyBox load */
  document.addEventListener('click', function(e) {
    var swatch = e.target.closest('.swatch');
    if (!swatch) {
      setTimeout(clearSizeSelection, 10);
    }
  });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      observer.observe(document.body, { attributes: true, subtree: true, attributeFilter: ['checked'] });
    });
  } else {
    observer.observe(document.body, { attributes: true, subtree: true, attributeFilter: ['checked'] });
  }
